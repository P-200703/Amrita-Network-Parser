...
if (len(sys.argv) < 2 or len(sys.argv) > 2):
    ...
    with open(sys.argv[1]) as csv_file:
        ...
        if choice == 1:
            print("Enter hwo many ports u want to evaluate:")
            n=int(input())
            ports=[]
            for i in range(0,n):
                print("enter port",i+1,'number:')
                kk=str(input())
                ports.append(kk)
            for row in reader:
                if(row[4] in ports):
            ...
            print("Pls input the time stamps below ")
            a=int(input("Enter the start time in epchos time"))
            b=int(input("Enter the end time in epchos time"))
